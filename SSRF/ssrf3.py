...
def board_write():
    ...
    body = request.form.get("body", "") # body 값을 form 데이터에서 가벼옵니다.
    data = f"title={title}&body={body}&user={session['idx']}" #전송 할 데이터를 구성합니다.
    response = request.post(f"{INTERNAL_API}/board/write",headers=header, data=data) # INTERNAL API 에 이용자가 입력한 값을 HTTP BODY 데이터로 사용해서 요청합니다.
    return response.content # INTERNAL_API 의 응답 경과를 반환합니다.
...
@app.route("/")
def index():
    # board_write 기능을 호출하기 위한 페이지 입니다
    return """
